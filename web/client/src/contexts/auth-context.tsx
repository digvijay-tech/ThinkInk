...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    if (!account || !signature || !token) {
      ...
      try {
        ...
        if (!auth.account || !auth.signature || !auth.token) throw new Error("JSON Parsing Error: Unidentified auth payload.");
  ...
  return (
    <AuthContext.Provider value={{ account, signature, isAuthenticated, setAuthState, removeAuthState, createdAt, lastLogin }}>{children}</AuthContext.Provider>
  );
};
